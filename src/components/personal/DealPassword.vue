<template>
  <div class="exchanged">
    <!-- 头部 -->
    <van-nav-bar left-arrow title="修改交易密码" @click-left="routerBack" />
    <div class="content">
      <van-cell-group>
        <div class="input_title">输入新交易密码：</div>
        <van-field v-model="password" placeholder="必须为六位数字" />
        <div class="input_title">确认新交易密码：</div>
        <van-field v-model="passwordConf" placeholder="必须为六位数字" />
  ...
  methods: {
    ...
    Determine() {
      ...
      if (this.password.trim() == "") {
        this.$toast("请输入交易密码");
        return;
      } else if (!second.test(this.password)) {
        this.$toast("交易密码为6位数字");
        return;
      }
